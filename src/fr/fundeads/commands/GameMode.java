			...
			if(p.hasPermission("fundeads.gm")) {
				...
				if(args.length == 2) {
					...
					if(target != null) {
						...
						if(args[0].equalsIgnoreCase("0") || args[0].equalsIgnoreCase("survival")) {
							p.sendMessage("§e[FunDeads] §9Passage En Mode Survie Pour §e" + target.getName());
							...
							p.sendMessage("§e[FunDeads] §9Passage En Mode Spectateur Pour §e" + target.getName());
				...
				p.sendMessage("Vous n'avez pas acc�s � cette commande !");
